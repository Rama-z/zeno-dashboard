...
export function renderAuthScreen(view: AuthView, state: AuthScreenState) {
  ...
  return `<main class="auth-shell"><section class="auth-brand-panel"><div class="auth-brand-lockup"><img src="/zeno-logo-96.webp" width="44" height="44" alt="Zeno" /><div><strong>Zeno</strong><span>PERSONAL WORKSPACE</span></div></div><div class="auth-brand-copy"><p class="eyebrow">SECURE BY DEFAULT</p><h2>Aktivitas, data, dan progres tersimpan atas nama Anda.</h2><p>Setiap perubahan tercatat, setiap record memiliki owner, dan akses hanya tersedia setelah verifikasi email.</p><div class="auth-security-list"><span>✓ Password bcrypt</span><span>✓ Email verification</span><span>✓ HttpOnly session</span><span>✓ Role-based access</span></div></div></section><section class="auth-form-panel">${view === 'register' ? register : view === 'verify' ? verify : login}</section></main>`;
}
